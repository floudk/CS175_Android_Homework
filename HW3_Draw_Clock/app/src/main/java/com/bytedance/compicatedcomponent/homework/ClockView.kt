...
//      drawHoursValues(canvas)
    ...
    private fun drawDegrees(canvas: Canvas) {
        ...
        val paint_text = Paint(Paint.LINEAR_TEXT_FLAG).apply {
...
//    }
     ...
     * @param canvas
     */
    private fun drawNeedles(canvas: Canvas, now: Date) {
//        val calendar: Calendar = Calendar.getInstance()
//        val now: Date = calendar.time
...
//        val nowSeconds: Int = now.seconds
    ...
    private fun drawBox(canvas: Canvas,now:Date) {
        val paint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            style = Paint.Style.FILL_AND_STROKE
            strokeCap = Paint.Cap.ROUND
            strokeWidth = resultWidth * DEFAULT_DEGREE_STROKE_WIDTH
        ...
        val leftx = centerX.toFloat()-100
        val lefty = centerY.toFloat()+130
        val length = 200+20
        val hight = 100
        canvas.drawRect(leftx,lefty,leftx+length,lefty+hight,paint)
        val paint_text = Paint(Paint.LINEAR_TEXT_FLAG).apply {
            color =Color.WHITE
            typeface= Typeface.create(Typeface.SANS_SERIF, Typeface.BOLD)
            textSize=DEFAULT_TEXT_SIZE+15
            alpha = 95
        }
        val current = now.hours.toString()+":"+now.minutes.toString()
        canvas.drawText(current,leftx+20,lefty+75,paint_text)
    }
